...
class CutTypeIndex(object):
    '''
    xpeng: CutTypeIndex has two field: samples and indexer
...
class Sample():
    ...
    #xpeng: iterate all the sites in breadth first search
    def sample(self):
        top_down_sites = list(self.sites()) #Turn generator to a list of tuple
        bottom_up_sites = top_down_sites
        bottom_up_sites.reverse() # This will also reverse top_down_sites
        assert bottom_up_sites[len(bottom_up_sites)-1][1] is None
        for node, parent in bottom_up_sites:
...
if __name__ == '__main__':
    ...
    while iteration <= FLAGS.iter:
        ...
        iteration += 1
        #if iteration % 50 == 0:
        #    filename = str(iteration) + "_iteration_file.eps"
        #    plot_likelihood(likelihoods, filename,
