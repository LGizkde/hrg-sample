...
class Sample():
    ...
    def __init__(self, hg, sent_num, SAMPLER=None):
        ...
        for node in self.hg.nodes:
            ...
            #if node != self.hg.root:
            #node.nosample = False #newly changed
            #    node.cut = random.randint(0,1)
            if node.nosample:
    ...
    def extract_one_rule(self, curr_node, children, ext_list, first_rule=False):
        ...
        #if FLAGS.href:
        #    suffix = self.get_node_externals(curr_node, ext_list)
        ...
        if curr_node.noprint and children is None:
            print 'discared rule: %s' % filter_vars(new_rule.dumped_format())
