...
gflags.DEFINE_bool(
    'keep_unigram',
    False,
    'Always keep unigram rules.')
gflags.DEFINE_integer(   #This flag is newly added by xpeng, mainly used to prune the maximum nonterminal type
    'max_type',
    8,
    'Discard rules that have more than 8 external nodes.')
logger.level = 1
        ...
        if r is not None:
            if sumrule is not None and r == sumrule:
                ...
                if sumrule is not None and sumrule.feats[0] >= FLAGS.count_cut:
                    if n_sum > FLAGS.discard_raw_count or (FLAGS.keep_unigram and sumrule.arity == 0 and len(sumrule.f) == 1):
    ...
    for r in gram:
        ...
        try:
            r.feats = feature_vector(r.feats[0],
                                     ...
                                     r.feats[1:3],
                                     r.feats[3:])
            match = re.search('\[A([0-9]{3})', str(r))
            #assert FLAGS.max_type > 10, 'weird'
            if FLAGS.max_type < 10:
                match = re.search('\[A([%d-9]|[0-9]{2})' % FLAGS.max_type, str(r))
            if not match:
                output_file.write("%s\n" % r)
