...
gflags.DEFINE_boolean(
    'slave',
    False,
    'If the current node is to be run')
gflags.DEFINE_string(
    'current',
    'node',
    'the current node to be run')
gflags.DEFINE_list(
    'nodes',
    'node82,node83,node84,node85',
    'The nodes to run this program')
def timed(l):
    prev = time.time()
...
def choose_m(n, c1, c2, sampler):
    ...
    for i in xrange(n+1):
        weights[i] += lncr(n, i)
    if FLAGS.anneal and not cease_update:
        weights = [w * FLAGS.annealing_factor for w in weights]
...
def choose_m(n, c1, c2, sampler):
    count = 0
    weights = [1.0 for x in xrange(2)]
    for i in xrange(n):
        for x in c1:
            weights[0] *= sampler.posterior(x)
        for x in c2:
            weights[1] *= sampler.posterior(x)
            sampler.count(x)
        if FLAGS.anneal and not cease_update:
            weights = [w ** FLAGS.annealing_factor for w in weights]
...
if __name__ == '__main__':
    ...
    #FLAGS.dump = FLAGS.dump+ '_'+ FLAGS.current
    ...
    flagfile.close()
    logger.writeln('Annealing factor %.2f, stepsize %.2f, ceiling %.2f, every %d iterations' % (FLAGS.annealing_factor, FLAGS.annealing_stepsize, FLAGS.annealing_ceiling, FLAGS.annealing_intervals))
    ...
    while iteration <= FLAGS.iter:
        ...
        #interval = (int)((n_sample+ FLAGS.annealing_intervals-1)/ FLAGS.annealing_intervals)
        #counts = 0
        for s in timed(samples):
            s.sample()

        if FLAGS.anneal and (not cease_update) and iteration % FLAGS.annealing_intervals == 0:
            if ascent:
                ...
                if (FLAGS.annealing_factor - FLAGS.annealing_stepsize < 1.0) or abs(FLAGS.annealing_factor - FLAGS.annealing_stepsize - 1.0) < 1e-4: #descent to a number smaller or equal to 1.0
                    ...
                    cease_update = True #Mark the end of annealing factor
                    logger.writeln('Current annealing factor: %.6f at iteration %d' % (FLAGS.annealing_factor, iteration))
                else:
                    FLAGS.annealing_factor -= FLAGS.annealing_stepsize
                    FLAGS.annealing_factor = (float)('%.2f' % FLAGS.annealing_factor)
    ...
    '''
        ...
        for curr_node in FLAGS.nodes:
            cmd = ' '.join(sys.argv)
            options = '--slave --current=%s' % curr_node
            os.system(r'ssh %s "cd %s; nohup %s %s >& %s.log" &' % (curr_node, os.getcwd(), cmd, options, curr_node))
    '''
