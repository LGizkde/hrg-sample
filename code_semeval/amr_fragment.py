...
def combine_fragments(f1, f2, refine=False):
    ...
    if refine:
        for index in ext_set:
            new_frag.ext_label[index] = f1.ext_label[index] if index in f1.ext_set else f2.ext_label[index]
    ...
    for node_index in common_set:
        if new_frag.is_ext(node_index):
            ...
            if refine:
                curr_node = amr_graph.nodes[node_index]

                if new_frag.edges[curr_node.c_edge] == 1: #If this node has its concept label
                    new_frag.ext_label[node_index] = f1.ext_label[node_index] if f1.edges[curr_node.c_edge] == 1 else f2.ext_label[node_index]
                else:
                    for p_edge_index in curr_node.p_edges:
                        if new_frag.edges[p_edge_index] == 1:
                            new_frag.ext_label[node_index] =  str(amr_graph.edges[p_edge_index])
                            break
