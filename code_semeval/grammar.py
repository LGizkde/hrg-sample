...
class Grammar(object):
    def __init__(self, rule_bin_size):
        self.rule_bin_size = rule_bin_size
        self.name = 'local_gr'  # added by freesunshine
        self.reset()
    ...
    # added by freesunshine, for directly initializing SCFG grammar from a set of rules
    def build(self, rules, features):
        if logger.level >= 1:
            logger.writeln('building grammar from %d rules' % len(rules))
        for r in rules:
            r.grammar = self
            features.score_rule(r)
            self.add(r)
