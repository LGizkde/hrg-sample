...
single_score=False #global variable, single score output control

pr_flag=True #global variable, output precision and recall
...
def sent_smatch(cur_amr1, cur_amr2, sent_num):
  while True:
     #cur_amr1=get_amr_line(args.f[0])
     #cur_amr2=get_amr_line(args.f[1])
     #sent_num += 1
     if not cur_amr2 or not cur_amr2:
         break
     #print cur_amr1
     #print cur_amr2
     ...
     if(cur_amr2==""):
      ...
      # continue
     amr1=amr_old.AMR.parse_AMR_line(cur_amr1)
     amr2=amr_old.AMR.parse_AMR_line(cur_amr2)
     test_label="a"
     gold_label="b"
     ...
     try:
       ...
       #print >> sys.stderr, 'error amr %s' % cur_amr2
       #print >> sys.stderr, 'correct amr %s' % cur_amr1
       sent_num += 1
       continue
   ...
   #    print >> sys.stderr, gold_rel
     ...
     if best_f_score < 0.5:
        print cur_amr1
        print cur_amr2
     return (best_f_score, best_match_num, len(test_rel1)+len(test_inst)+len(test_rel2),len(gold_rel1)+len(gold_inst)+len(gold_rel2))
  return None
...
def main(args):
  ...
  while True:
     ...
     cur_amr2=get_amr_line(args.f[1])
     sent_num += 1
     smatch_result = sent_smatch(cur_amr1, cur_amr2, sent_num)
     if not smatch_result:
         break
 #    if not cur_amr2 or not cur_amr2:
 #        break
 #    #print cur_amr1
