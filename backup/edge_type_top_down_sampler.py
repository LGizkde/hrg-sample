#!/u/xpeng/pypy/pypy-2.1/pypy-2.1-beta-src/pypy-c
...
def choose_split(n, choices, correction, sampler):
    choice_prob = [SAMPLER.choice_posterior(c)*f
    ...
    for sample in timed(samples):
        ...
        for n, rule in sample.composed_rules_under(sample.hg.root):
            ...
            if FLAGS.type_cut:
                ...
                cut_type = rule.key()
                CUT_INDEX.add(cut_type, sample, n)
                n.composed_rule = rule #xpeng: Why this part is needed?
            if FLAGS.type_edge:
                edge_type = sample.node_edge_type(n)
                EDGE_INDEX.add(edge_type, sample, n)
...
class TypeIndexer(object):
    '''
    xpeng: TypeIndexer has two field: samples and indexer
    samples: all the sentences(hypergraph-structured) to be sampled from
    indexer: from rule to sites, each site of (sample, node) pair structure
    '''
    def __init__(self):
    ...
    '''
    get all the sites according to the rule
    '''
    def get(self, key):
        #print('get %s' % rule)
        #key_value = rule.key()
        sites = self.index.get(key)
        if sites is None:
            return []
        else:
            return sites

    def add(self, key, sample, node):
        #print('add %s' % rule)
        self.index.setdefault(key, set()).add((sample, node))

    def remove(self, key, sample, node):
        #print('remove %s' % rule)
        #print('key:')
        #print(sample)
        #print('at node %s %s' % (node, sample.make_composed_rule(node)))
        nodeset = self.index.get(key)
        #print('in node set')
        #for s, n in nodeset:
        #    if (s is sample):
        #        print(s)
    ...
    '''
    xpeng: this method does two things:
    1.iterate all composed rules under the root node and make sure the same node does not yield twice by composed_rule_under
    2.iterate all indexed sites correlated with the current sample, make sure that the same node does not appear twice, and the indexed rule for the same node is the current rule
    '''
    def check(self, sample):
        ...
        for n, r in sample.composed_rules_under(sample.hg.root):
            ...
            node2rule[n] = r.key()
        node2rule_index = {}
        for key, nodeset in self.index.iteritems():
            for s, n in nodeset:
                if s is sample:
                    assert n not in node2rule_index, \
                            '%s\n%s is duplicated in index' % (s, n)
                    node2rule_index[n] = key
                    assert key == node2rule[n], \
                            '%s\n%s\nrule in index: %s actual rule: %s' % (s, n, key, node2rule[n])
...
class Sample():
    ...
    def __init__(self, hg, a):
        ...
        for node in self.hg.nodes:
            ...
            # node.sampled = False
            node.cut_type_sampled = False
            node.edge_type_sampled = False
    ...
    '''
    xpeng: edge types for node, including all the branches, sorted order
    '''
    def node_edge_type(self, node):
        old = node.edge
        n_edges = len(n.incoming)
        if n_edges == 0: #leaf nodes, no child
            rule = self.composed_rules_under(node)
            return rule.key()

        edge_type = []
        for i in xrange(n_edges):
            node.edge = i
            edge_type.append(self.branch_type(node))
        edge_type.sort()
        return tuple(edge_type)

    '''
    xpeng: branch type for choose one branch of the node
    To be verified: parent rules are concatenated with sorted child rules
    '''
    def branch_type(self, node):
        ...
        rule = make_rule([node.fi, node.fj, node.ei, node.ej],
                         ...
                         get_nt(node),
                         [get_nt(c) for c in mychildren])
        branch_type.append(rule.key())
        for child in mychildren:
            branch_type.append(self.node_edge_type(child))

    '''
    the cut type is decided by the composed-rule rooted at its parent, with the current node cut-type as 0
    '''
    ...
    def sample_edge(self, node, parent):
        if parent is None:  # root node
            parent = node
            #logger.writeln('check the root node')
            #if len(node.incoming) > 1:
            #    logger.writeln('root has more than 1 incoming edges')
        ...
        if FLAGS.type_cut:
            ...
            for n, r in self.composed_rules_under(parent):
                cut_type = r.key()
                CUT_INDEX.remove(cut_type, self, n)

            node.edge = i
            for n, r in self.composed_rules_under(parent):
                cut_type = r.key()
                CUT_INDEX.add(cut_type, self, n)
                n.composed_rule = r

        if FLAGS.double_count:  #xpeng: to recover from the previous double_discount
    ...
    #xpeng: I implement this part using top down strategy
    def type_sample_edge(self, node, parent):
        global EDGE_SITE_COUNT
        global EDGE_SITE_CHECK
        global EDGE_SITE_CHECK_SUCCESS
        if parent is None:  # root node
            parent = node
        ...
        #xpeng: The edge type starts with the smallest cut
        if node.cut:
            parent = node
        ...
        density_factors = []
        n_edges = len(node.incoming)
        for i in range(n_edges):
            node.edge = i
            rule_lists.append([r for n, r in self.composed_rules_under(parent)])
            density_factors.append(density_factor(node))

        edge_type = self.node_edge_type(node)
        node.edge = old

        #There is no need to track the path as in the type_sample_cut
        edge_sites = [[] for x in xrange(n_edges)] #This is differentiate which edge is selected
        edge_conflict_tester = EdgeConflictTester() #This remains to be implemented, conflict nodes to be verified
        assert edge_conflict_tester.ok(node)

        #deal with the current site
        edge_sites[node.edge].append((self, node))

        for s, n in EdgeIndexer.get(edge_type):
            if edge_conflict_tester.ok(n):
                edge_sites[n.edge].append((s,n))

        sites = []
        n_edge_sites = [len(s) for s in edge_sites]
        for i in xrange(n_edges):
            sites = sites + edge_sites[i]
        n_sites = len(sites)
        assert sum(n_edge_sites) == n_sites, 'The number of edge sites does not correspond'

        #keep some stats
        EDGE_SITE_COUNT[n_sites] = EDGE_SITE_COUNT.get(n_sites, 0)+ 1

        #Here are some discounts about old edges needed (go over all the composed rules)
        for i in xrange(n_edges):
            for _ in xrange(n_edge_sites[i]):
                for rule in rule_lists[i]:
                    SAMPLER.discount(rule)

        #A more difficult part, multiple value instead of binary, how to sample
        dists = choose_split(n_sites, rule_lists, density_factors, SAMPLER)


        #Here are some counts on new edges needed
        for i in xrange(n_edges):
            site_num = dists[i]
            for _ in xrange(site_num):
                for rule in rule_lists[i]:
                    SAMPLER.discount(rule)
    ...
    def type_sample_cut(self, node, parent):
        ...
        for s, n in CUT_INDEX.get(r3_key):
            SITE_CHECK += 1
            # skip root nodes
        ...
        for s, p in CUT_INDEX.get(r1_key):
            SITE_CHECK += 1
            if s is not self:
                n = p
                error = False
        ...
        for i, site in enumerate(sites):
            ...
            if i < n_r1 and not r1_test[i]:
                ...
                p = cut_parent(n)
                #r1_key = r1.key()
                CUT_INDEX.remove(r1_key, s, p)
                n.cut = 1
                #r2_key = r2.key()
                #r3_key = r3.key()
                CUT_INDEX.add(r2_key, s, p)
                CUT_INDEX.add(r3_key, s, n)
                p.composed_rule = r2
                n.composed_rule = r3
            # flip cut to uncut
            if i >= n_r1 and r1_test[i]:
                ...
                #r2_key = r2.key()
                #r3_key = r3.key()
                CUT_INDEX.remove(r2_key, s, p)
                #print('remove rule under self')
                CUT_INDEX.remove(r3_key, s, n)
                n.cut = 0
                #r1_key = r1.key()
                CUT_INDEX.add(r1_key, s, p)
                p.composed_rule = r1
    ...
    def sample(self):
        ...
        while len(queue) > 0:
            ...
            if FLAGS.sample_level is None or node.level <= FLAGS.sample_level:
                if FLAGS.type_edge:
                    self.type_sample_edge(node, parent)
                else:
...
if __name__ == '__main__':
    ...
    likelihood_handle = open(likelihood_file, 'w')
    ...
    CUT_INDEX = TypeIndexer()
    EDGE_INDEX = TypeIndexer()
    ...
    while iteration <= FLAGS.iter:
        ...
        '''
        if iteration % 10 == 0:
            iter_likelihood_file = FLAGS.dump+ '/'+ str(iteration) + 'iter_likelihood.eps'
            title = 'iter-loglikelihood'
            plot_likelihood(likelihoods, iter_likelihood_file, title)
        '''
