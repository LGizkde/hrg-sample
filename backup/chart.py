...
gflags.DEFINE_boolean(
    'use_simple_bin',
    False,
    'Use simple bin instead of self-pruning bin.')
...
class Bin(object):
    ...
    def __getitem__(self, i):
        """This is the interface with cube pruning. Cube pruning takes this bin
        as a sorted list."""
        if not hasattr(self, 'sorted_items'):
            self.sorted_items = [item for negcost, item in self.items if not item.dead]
...
class SimpleBin(object):
    "A simple non-pruning bin. With cube-pruning this simple bin can be used."
    def __init__(self, K, chart):  # K is ignored
        self.chart = chart
        self.items = []
        self.ndead = 0  # only for interfacing purposes
    ...
    def __getitem__(self, i):
        """This is the interface with cube pruning. Cube pruning takes this bin
        as a sorted list. merging happens here."""
        if not hasattr(self, 'sorted_items'):
            merged_items = {}
            for item in self.items:
                old = merged_items.get(item)
                if old is None:
                    merged_items[item] = item
                else:
                    item_merged = item.merge(old)
                    if item_merged:  # old item better
                        pass
                    else:
                        merged_items[item] = item
                    self.chart.merged += 1
            self.sorted_items = merged_items.values()
            self.sorted_items.sort()
        return self.sorted_items[i]
...
class Chart(object):
    def __init__(self, fwords, start_symbol):
        ...
        if FLAGS.use_simple_bin:
            self.binclass = SimpleBin
        else:
            self.binclass = Bin
    ...
    def add(self, item):
        ...
        if bin_idx:  # discard items with None key
            bin = self.bins.setdefault(bin_idx, self.binclass(FLAGS.bin_size, self))
            # preprune
