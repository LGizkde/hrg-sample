...
class FE(Feature):
    ...
    def weight(self, rule):
      return -elog10(rule.feats[0])

class Count(Feature):
    def weight(self, rule):
        return 1

#This is to punish the rules that has too many edges
class EdgeCount(Feature):
    def weight(self, rule):
        count = 0
        for tok in rule.f:
            if isvar(tok):
...
#This is to punish the rules with large numbers of tails
#[A7-0] --> 7
class RuleType(Feature):
    def weight(self, rule):
        if rule.lhs.find('[A') != -1:
            return int(rule.lhs[1:-1].split('-')[0][1:])
        else:
            return 0

#This feature is to reward rules with more terminals on source side
class NumTerminals(Feature):
    def weight(self, rule):
        num_terminals = 0
        for tok in rule.f:
            if isvar(tok):
