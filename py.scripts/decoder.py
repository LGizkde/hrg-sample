...
gflags.DEFINE_list(
    'available_nodes',
    None,
    'If this options is provided then no probing on node is run. Example: f02:2,f03,f04:4')
gflags.DEFINE_string(
    'output',
    'output',
...
def main():
    ...
    #print >>sys.stderr, "================="
    #print >>sys.stderr, FLAGS.kbest_output
    if FLAGS.parallel:  # as master
        available_nodes = []
        if FLAGS.available_nodes is None:
            # probe nodes
            ...
            for elem in FLAGS.available_nodes:
                if ':' in elem:
                    node, num = elem.split(':')
                    for _ in range(int(num)):
                        available_nodes.append(node)
                else:
                    available_nodes.append(elem)
