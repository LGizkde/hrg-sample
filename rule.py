...
rule_f = open(os.path.join(os.getcwd(), 'hrg_error.log'), 'a')
def nt_escape(sym):
...
def reform_edge(s):
    ...
    while position < len(s):
        ...
        try:
            ...
            print 'Error happened in this sentence: %s    %s' % (s, token)
            sys.exit(0)
    ...
    def compose(self, rules):
        ...
        for i,sub_rule in enumerate(rules):
            ...
            try:
                ...
                print str(fragment)
                print str(sub_e)
                print self.dumped_format()
                #print>> rule_f, self.dumped_format()
