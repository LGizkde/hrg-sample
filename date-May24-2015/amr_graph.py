...
class AMRGraph(object):
    ...
    def recall_unaligned_concepts(self, edge_alignment, unaligned_toks, lemma_map, stop_words):
        n_nodes = len(self.nodes)
        n_edges = len(self.edges)
            ...
            if edge_alignment[c_edge_index] == 0:
                ...
                if len(curr_node.v_edges) > 0:
                    for curr_edge_index in curr_node.v_edges:
                        ...
                        tail_node_index = curr_edge.tail
                        if is_root_arc(curr_edge.label):
                ...
                if len(root_arcs) != 0:  #There are arguments going out of the current node
...
def match_word(label, unaligned_words, lemma_map, stop_words):
    for (pos, word) in unaligned_words:
        ...
        if len(label) > 4:
            ...
            if word[:2] == label[:2] or (lem_w and lem_w[:2] == label[:2]):
                return (pos, word)
    return (None, None)
