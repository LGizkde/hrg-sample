...
def output_all_unaligned_edges(edge_alignment, amr_graph):
    ...
    print >> unalign_f, ' '.join(un_seq)
            #print >> unalign_f, str(amr_graph.edges[i])

#Build one node that would cover unaligned edges going out of a node
def build_one_node(curr_frag, curr_start, curr_end, amr_graph, edge_alignment):
    ...
    if len(root_arcs) > 0:
        ext_set = set()
        ext_set.add(curr_node_index) #Not necessarily true
...
def fragment_decomposition_forest(fragments, amr_graph, unaligned_fragments, edge_alignment):
    ...
    for i in xrange(n):
        ...
        new_node = build_one_node(frag, i, j, amr_graph, edge_alignment)
        filter_with_maxtype(new_node)
            ...
            for node1 in curr_candidate:
                for unaligned_node in unaligned_nodes:
                    ...
                    if check_disjoint(node1.frag, unaligned_node.frag):
                        (new_frag, connect_frags) = general_combine_fragments(node1.frag, unaligned_node.frag, edge_alignment)
                        if new_frag is None:
                            continue
                ...
                for node1 in chart[i][k]:
                    for node2 in chart[k][j]:
                        ...
                        if check_disjoint(node1.frag, node2.frag):
                            ...
                            (new_frag, connect_frags) = general_combine_fragments(node1.frag, node2.frag, edge_alignment)
...
def parallel_forest_construct(argv):
    if argv[1] == '-m': #The main processor
        ...
        for sent_index in xrange(sent_no):
            ...
            hg = fragment_decomposition_forest(aligned_fragments, amr_graph, unaligned, edge_alignment)
            if amr_graph.check_self_cycle():
                logger.writeln(str(amr_graph))
            elif hg is not None:
